#!/usr/bin/env python3
"""Replay and compare the local single-admin-unit validation bundle."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--workdir", type=Path, default=Path("artifacts/sadv-replay"))
    parser.add_argument(
        "--evidence-dir",
        type=Path,
        default=Path("docs/vignettes/sadv/evidence"),
    )
...
def display_path(path: str | Path) -> str:
    """Use repository-relative paths in human-facing reports when possible."""
...
def safe_extract_zip(zip_path: Path, extract_root: Path) -> Path:
    if not zip_path.is_file():
        raise FileNotFoundError(f"validation zip was not found: {zip_path}")
...
def _run_batch(bundle_root: Path) -> Path:
    ...
    if not runs:
        raise FileNotFoundError("could not find validation run batch file")
    return runs[0]
...
def write_summary_markdown(summary: dict[str, Any], target: Path) -> None:
    lines = [
        "# Single Administrative Unit Validation Comparison",
        "",
        f"- Created UTC: `{summary['created_utc']}`",
        f"- Zip: `{display_path(summary['zip'])}`",
        f"- Workdir: `{display_path(summary['workdir'])}`",
        f"- Bundle root: `{display_path(summary['bundle_root'])}`",
...
def run_replay(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if bundle_root is None:
        raise FileNotFoundError(f"could not find validation bundle root under {extracted_root}")
